...
// 【filesystem/resolver】是为了方便跨平台处理文件路径用的，不同的操作系统都能找到这个文件
...
{
    std::cout << "【测试】本代码是从main函数生成一个可执行文件然后取消它的权限，使其只能在终端手动运行，如此允许输入额外的参数。\n";
    if (argc < 2) {
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        try
        {
            if (path.extension() == "xml")
            {
                ...
                // getFileResolver()返回一个filesystem::resolver指针，这里直接使用了该指针指向的resolver类的prepend函数，
                // 但是还是不清楚为啥这里prepend了一下之后就啥也没干了，并且这个指针没有赋值给任何指针变量，为啥会有什么“长期性”
                // 影响呢？
    ...
    if (sceneName != "")
    {
        ...
        try
        {
            // unique_ptr是智能指针，root是一个指向NoriObject的指针，是整个渲染管线的根节点
            std::unique_ptr<NoriObject> root(loadFromXML(sceneName));
            /* When the XML root object is a scene, start rendering it .. */
            // 如果不是scenes就do nothing
            if (root->getClassType() == NoriObject::EScene)
                // static_cast将用智能指针的内容提取出来转化为Scene，智能指针自动析构
